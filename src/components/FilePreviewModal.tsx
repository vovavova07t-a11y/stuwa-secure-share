...
import { Download, X, ZoomIn, ZoomOut, RotateCw, RefreshCw, AlertCircle, ExternalLink } from 'lucide-react';
...
export const FilePreviewModal: React.FC<FilePreviewModalProps> = ({
  ...
  const [previewMode, setPreviewMode] = useState<'iframe' | 'embed' | 'image' | 'download'>('iframe');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Определяем режим предпросмотра на основе типа файла
      if (fileType === 'pdf') {
  ...
  const handleRetry = () => {
    ...
    // Переключаем режим предпросмотра для повторной попытки
  ...
  const renderPreviewContent = () => {
        ...
        <div className="w-full h-[70vh] border rounded-lg overflow-hidden bg-white relative">
          ...
          {previewMode === 'iframe' ? (
            <iframe
              ...
              onError={() => {
                setIsLoading(false);
                setError('Не удалось загрузить PDF в iframe. Попробуем альтернативный способ.');
                setPreviewMode('embed');
              }}
              style={{ 
    ...
    // Для других типов файлов показываем опции загрузки
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] bg-gray-50 rounded-lg">
        ...
        <p className="text-gray-600 mb-4 text-center max-w-md">
          ...
          Используйте одну из опций ниже для работы с файлом.
        </p>
        <div className="flex gap-3">
          <Button onClick={handleOpenInNewTab} variant="outline">
            <ExternalLink className="w-4 h-4 mr-2" />
            ...
            <div className="flex-1 min-w-0">
              ...
              {fileSize && (
                <p className="text-sm text-gray-600 mt-1">
                  Размер: {formatFileSize(fileSize)} • Тип: {fileType.toUpperCase()}
                </p>
              )}
            </div>
